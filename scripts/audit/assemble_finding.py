...
def render_document(
    ...
    lines = [
        ...
        "states, and the artifact integrity they rest on. How much that proves has limits; two "
        "are worth naming here. First, the first criterion's field-by-field half runs against "
        "a mapping from that line's prose onto findings keys, hand-authored inside the gate "
        "and marked there as a reading rather than a measurement -- and two of the fields it "
        "maps are empty, passing by exemptions the gate declares by name: "
        "`qcew_routes.bulk_years_required`, an empty list the plan calls a legitimate finding, "
        "and `cbp_metadata.lfo_by_year`, null for every window year, whose why is the §1.2 row "
        "below. Second, the criterion covering §1.2's final bullet and the three §21 rows is "
        "four substring matches plus one presence test per Appendix A source name: its pass "
        "says those sections are in this document, not that their verdict cells say anything, "
        "so read the verdicts rather than that PASS line. "
