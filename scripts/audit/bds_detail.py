...
def compose_retention_rule(extract_statuses: list[int]) -> dict:
    ...
    return {
        "rule": (
            ...
            "probe a non-200 body IS the evidence: a 204 with an empty body (matched no "
            "published cell), a 200 auth-rejection HTML page, and a 200 real tabular answer are "
            "three different verdicts and only the retained bytes tell them apart. A transport "
