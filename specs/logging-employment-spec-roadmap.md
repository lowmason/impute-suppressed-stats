...
- [ ] Stage 4: Pseudo-suppression validation harness and baseline scoreboard
      ...
      RE-VALIDATED against Stage 3 as shipped: still routes to writing-plans, with three additions. A mask that changes the partition MUST rebuild `EstimatorContext.partitions` from the same mask it hands the anchor — `baselines.simple.disclosed_intensity` reads the context's partition while the residual comes from the anchor argument, and nothing yet enforces that they agree (see specs/deferred_items.md). Scoring must read `weight_basis_counts` per estimator, not pooled, or a composite's score is reported as a pure estimator's. And the preferred-baseline slot Stage 4 fills should use `preferred_estimator_by_month`: the window scalar names `cbp_intensity` even for the 12 months of 2024 in which it produced nothing.
